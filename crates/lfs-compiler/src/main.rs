...
fn run() -> Result<(), String> {
    ...
    if args.len() < 4 || args[1] != "compile" {
        return Err("usage: local-first-state compile INPUT_DIR OUTPUT_DIR [--mutation-history FILE] [--initialize-mutation-history] [--schema-fence FILE]".into());
    }
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--mutation-history" | "--schema-fence" => {
                let value = args.get(index + 1).ok_or("missing option path")?;
                if args[index] == "--mutation-history" {
                    history_path = PathBuf::from(value);
                    explicit_history = true;
                } else {
                    fence_path = PathBuf::from(value)
                }
                index += 1;
            }
    ...
    let mut config = lfs_compiler::compile(&source).map_err(|e| {
        let line = e
            .split(':')
            .next()
    ...
    if fence_path.exists() {
        lfs_compiler::check_fence(&read_json(&fence_path)?, &config["schema"])?;
    }
    ...
    let history = lfs_compiler::reconcile_history(&config, previous.as_ref())?;
    ...
    let files = [
        ...
        (out.join("generated.ts"), lfs_compiler::typescript(&config)),
        (out.join("generated.dart"), lfs_compiler::dart(&config)),
        (
            history_path,
