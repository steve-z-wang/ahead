...
fn ordered_slot_decodes_known_fields_and_ignores_new_fields() {
    let args=lfs_server::decode_arguments(&config(),&json!({"name":"edit","operations":[{"model":"Task","op":"update","identity":{"id":"a","future":1},"values":{"title":"hi","future":true}}]})).unwrap();
    assert_eq!(
        args,
...
fn known_disallowed_patch_is_explicit_refusal() {
    let err=lfs_server::decode_arguments(&config(),&json!({"name":"edit","operations":[{"model":"Task","op":"update","identity":{"id":"a"},"values":{"note":"x"}}]})).unwrap_err();
    assert_eq!(err, "edit.not_allowed");
}
#[test]
fn undeclared_operation_is_invalid() {
    assert_eq!(lfs_server::decode_arguments(&config(),&json!({"name":"edit","operations":[{"model":"Task","op":"delete","identity":{"id":"a"}}]})).unwrap_err(),"mutation.invalid");
}
#[test]
fn create_binding_mismatch_refuses_the_whole_act() {
    ...
    assert_eq!(
        lfs_server::decode_arguments(&c, &body).unwrap_err(),
        "create_pair.invalid"
    );
}
#[test]
fn historical_known_field_outside_capability_is_refused() {
    ...
    let result = lfs_server::decode_arguments(
        &c,
...
fn live_subscribe_requires_one_subscribe_frame_and_normalizes_scopes() {
    let decoded = lfs_server::live::decode_subscribe(
        br#"{"type":"subscribe","scopes":["shared","alice","shared"]}"#,
    )
    .unwrap();
    assert_eq!(decoded, vec!["alice", "shared"]);
    assert!(lfs_server::live::decode_subscribe(br#"{"type":"other","scopes":["a"]}"#).is_err());
    assert!(lfs_server::live::decode_subscribe(br#"{"type":"subscribe","scopes":[]}"#).is_err());
}
...
fn live_page_progression_uses_wire_cursor_and_fifty_row_boundary() {
    ...
    let progress = lfs_server::live::page_progress(&full.to_string(), "shared", 7).unwrap();
    assert_eq!(progress.to_cursor, 57);
    assert!(progress.continues);

    let tail = json!({"scope":"shared","fromCursor":57,"toCursor":60,"changes":[]});
    let progress = lfs_server::live::page_progress(&tail.to_string(), "shared", 57).unwrap();
...
fn startup_rejects_invalid_patch_capabilities() {
    for fields in [json!(["id"]), json!(["missing"]), json!(["title", "title"])] {
        ...
        assert!(lfs_server::Config::decode(c).is_err());
    }
    let mut c = config();
