...
fn pull_copies_the_row_stamp_into_the_change() {
    ...
    let text = run(savoia_server::process_pull(
        &config(),
        "u",
        &pull_body(),
        &host,
    ...
    .unwrap();
    let page = savoia_core::PullPage::decode(text.as_bytes()).unwrap();
...
fn pull_rejects_rows_without_a_positive_stamp() {
    for bad in [Value::Null, json!(0), json!(-1), json!(9007199254740992u64)] {
        ...
        let err = run(savoia_server::process_pull(
            &config(),
            "u",
            &pull_body(),
            &host,
...
fn publish_requires_cursor_and_stamp_from_the_host() {
    ...
    run(savoia_server::publish(&config(), &changes, &channels, &ok)).unwrap();
    ...
    for bad in [json!(3), json!({"cursor":3}), json!({"cursor":3,"stamp":0})] {
        ...
        let err = run(savoia_server::publish(
            &config(),
            &changes,
            &channels,
            &host,
