...
impl<S: ClientStore> Engine<'_, S> {
    ...
    pub fn drop_record(&mut self, key: &RecordKey) -> Result<()> {
        self.exec(
            "otter_record",
            "DELETE FROM otter_record WHERE model=? AND identity=?",
            &[json!(key.model), json!(key.encoded_identity()?)],
        )?;
        Ok(())
    }
    pub fn claim_add(&mut self, channel: &str, key: &RecordKey) -> Result<()> {
        self.exec(
            "otter_claim",
            "INSERT OR IGNORE INTO otter_claim (channel, model, identity) VALUES (?,?,?)",
            &[
                json!(channel),
                json!(key.model),
    ...
    pub fn claim_remove(&mut self, channel: &str, key: &RecordKey) -> Result<()> {
        self.exec(
            "otter_claim",
            "DELETE FROM otter_claim WHERE channel=? AND model=? AND identity=?",
            &[
                json!(channel),
                json!(key.model),
                json!(key.encoded_identity()?),
    ...
    pub fn claims(&mut self, key: &RecordKey) -> Result<Vec<String>> {
        let rows = self.rows(
            "SELECT channel FROM otter_claim WHERE model=? AND identity=? ORDER BY channel",
            &[json!(key.model), json!(key.encoded_identity()?)],
        )?;
        Ok(rows
            .rows
    ...
    pub fn claims_remove_all(&mut self, key: &RecordKey) -> Result<()> {
        self.exec(
            "otter_claim",
            "DELETE FROM otter_claim WHERE model=? AND identity=?",
            &[json!(key.model), json!(key.encoded_identity()?)],
        )?;
        Ok(())
    }
    pub fn claimed_by(&mut self, channel: &str) -> Result<Vec<RecordKey>> {
        let rows = self.rows(
            "SELECT model, identity FROM otter_claim WHERE channel=? ORDER BY model, identity",
            &[json!(channel)],
        )?;
        rows.rows
            .into_iter()
    ...
    pub fn cursor(&mut self, channel: &str) -> Result<Option<u64>> {
        self.scalar(
            "SELECT cursor FROM otter_subscription WHERE channel=?",
            &[json!(channel)],
        )?
        .map(|v| as_u64(&v))
        .transpose()
    ...
    pub fn delete_subscription(&mut self, channel: &str) -> Result<()> {
        self.exec(
            "otter_subscription",
            "DELETE FROM otter_subscription WHERE channel=?",
            &[json!(channel)],
        )?;
        Ok(())
    }
    pub fn subscriptions(&mut self) -> Result<Vec<(String, u64)>> {
        let rows = self.rows(
            "SELECT channel, cursor FROM otter_subscription ORDER BY channel",
            &[],
        )?;
        rows.rows
            .into_iter()
