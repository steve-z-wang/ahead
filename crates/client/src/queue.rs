...
impl<S: ClientStore> Engine<'_, S> {
    fn bump(&mut self, column: &str) -> Result<u64> {
        let current = self
            .scalar(&format!("SELECT {column} FROM otter_client"), &[])?
            .ok_or_else(|| invalid("client row missing"))?;
        let value = as_u64(&current)?;
        let next = value
            .checked_add(1)
            .filter(|v| *v <= otter_core::MAX_SAFE_INTEGER)
            .ok_or_else(|| invalid("counter exhausted"))?;
        self.exec(
            "otter_client",
            &format!("UPDATE otter_client SET {column}=?"),
            &[json!(next)],
        )?;
        Ok(value)
    }
    ...
    pub fn insert_mutation(&mut self, ordinal: u64, mutation: &Mutation) -> Result<()> {
        self.exec(
            "otter_mutation",
            "INSERT INTO otter_mutation (ordinal, name, version, push) VALUES (?,?,?,NULL)",
            &[
                json!(ordinal),
                json!(mutation.name),
    ...
    pub fn add_effect(&mut self, ordinal: u64, op: &Operation) -> Result<()> {
        let next = self.scalar(
            "SELECT COALESCE(MAX(position), -1) + 1 FROM otter_mutation_operation WHERE ordinal=?",
            &[json!(ordinal)],
        )?;
    ...
    fn queued_where(&mut self, filter: &str, params: &[Value]) -> Result<Vec<Queued>> {
        let mutations = self.rows(
            &format!(
                "SELECT ordinal, name, version, push FROM otter_mutation {filter} ORDER BY ordinal"
            ),
            params,
        )?;
    ...
    pub fn dirty(&mut self, key: &RecordKey) -> Result<bool> {
        Ok(self
            .scalar(
                "SELECT 1 FROM otter_mutation_operation WHERE model=? AND identity=? LIMIT 1",
                &[json!(key.model), json!(key.encoded_identity()?)],
            )?
            .is_some())
    }
    pub fn delete_mutations(&mut self, ordinals: &[u64]) -> Result<()> {
        for ordinal in ordinals {
            self.exec(
                "otter_mutation",
                "DELETE FROM otter_mutation WHERE ordinal=?",
                &[json!(ordinal)],
            )?;
        }
        for table in [
    ...
    pub fn assign_push(&mut self, ordinals: &[u64], push: u64) -> Result<()> {
        for ordinal in ordinals {
            self.exec(
                "otter_mutation",
                "UPDATE otter_mutation SET push=? WHERE ordinal=?",
    ...
    pub fn prerequisite_keys(&mut self) -> Result<Vec<(String, Option<String>)>> {
        let rows = self.rows(
            "SELECT key, MAX(error) FROM otter_mutation_prerequisite GROUP BY key ORDER BY key",
            &[],
        )?;
        Ok(rows
            .rows
    ...
    pub fn resolve_prerequisite(&mut self, key: &str) -> Result<usize> {
        self.exec(
            "otter_mutation_prerequisite",
            "DELETE FROM otter_mutation_prerequisite WHERE key=?",
            &[json!(key)],
        )
    }
    pub fn fail_prerequisite(&mut self, key: &str, error: &str) -> Result<usize> {
        self.exec(
            "otter_mutation_prerequisite",
            "UPDATE otter_mutation_prerequisite SET error=? WHERE key=?",
            &[json!(error), json!(key)],
        )
    }
    pub fn reset_prerequisite(&mut self, key: &str) -> Result<usize> {
        self.exec(
            "otter_mutation_prerequisite",
            "UPDATE otter_mutation_prerequisite SET error=NULL WHERE key=?",
            &[json!(key)],
        )
    }
    pub fn checkpoints(&mut self, push: u64) -> Result<Vec<ChannelCheckpoint>> {
        let rows = self.rows(
            "SELECT channel, cursor FROM otter_push_checkpoint WHERE push=? ORDER BY channel",
            &[json!(push)],
        )?;
        rows.rows
            .iter()
    ...
    pub fn insert_checkpoints(
        ...
        for cp in checkpoints {
            self.exec(
                ...
                "INSERT INTO otter_push_checkpoint (push, channel, cursor) VALUES (?,?,?)",
    ...
    pub fn delete_checkpoints(&mut self, push: u64) -> Result<()> {
        self.exec(
            "otter_push_checkpoint",
            "DELETE FROM otter_push_checkpoint WHERE push=?",
            &[json!(push)],
        )?;
        Ok(())
    }
    ...
    pub fn insert_rejection(
        ...
        self.exec(
            "otter_rejection",
            "INSERT OR REPLACE INTO otter_rejection (ordinal, name, code, detail) VALUES (?,?,?,?)",
            &[
                json!(ordinal),
                json!(name),
                json!(code),
    ...
    pub fn rejections(&mut self) -> Result<Vec<Rejection>> {
        let rows = self.rows(
            "SELECT ordinal, code FROM otter_rejection ORDER BY ordinal",
            &[],
        )?;
        rows.rows
            .iter()
    ...
    pub fn delete_rejection(&mut self, ordinal: u64) -> Result<()> {
        self.exec(
            "otter_rejection",
            "DELETE FROM otter_rejection WHERE ordinal=?",
            &[json!(ordinal)],
        )?;
        Ok(())
    }
