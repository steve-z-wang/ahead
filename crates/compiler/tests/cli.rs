...
fn cli_retains_history_and_does_not_overwrite_on_break() {
    let root = std::env::temp_dir().join(format!("savoia-compiler-cli-{}", std::process::id()));
    ...
    .unwrap();
    let compile = || {
        Command::new(env!("CARGO_BIN_EXE_savoia"))
            .arg("compile")
            .arg(&input)
            .arg(&out)
            .output()
...
fn cli_writes_backend_ts_with_the_requested_runtime_import() {
    let root = std::env::temp_dir().join(format!("savoia-compiler-backend-{}", std::process::id()));
    let input = root.join("input");
    let out = root.join("out");
    fs::create_dir_all(&input).unwrap();
    fs::write(
    ...
    .unwrap();
    let status = Command::new(env!("CARGO_BIN_EXE_savoia"))
        .arg("compile")
        .arg(&input)
        .arg(&out)
        .arg("--backend-runtime")
