...
void main() {
  ...
  test(
    ...
    () async {
      final dir = await Directory.systemTemp.createTemp('savoia-dart-test-');
      final schema =
          jsonDecode(
      ...
      var client = await Client.open(
        ...
        libraryPath: Platform.environment['SAVOIA_LIBRARY']!,
      );
      try {
  ...
  test(
    ...
    () async {
      final dir = await Directory.systemTemp.createTemp('savoia-dart-close-');
      final schema =
          jsonDecode(
                await File('../../fixtures/schemas/entry.json').readAsString(),
              )
