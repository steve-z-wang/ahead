...
Counters are bounded to JavaScript's safe integer range by Rust, the adapter, and database constraints. Receipt text is stored verbatim and returned on an exact semantic retry. Table names use the `lfs_` prefix and are currently fixed. See [Server README](../server/README.md) for application wiring.
