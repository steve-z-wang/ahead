...
Counters are bounded to JavaScript's safe integer range by Rust, the adapter, and database constraints. Receipt text is stored verbatim and returned on an exact semantic retry. Table names use the `otter_` prefix and are currently fixed. `otter_record` holds one row per `(model, identity)` and allocates its stamp: every `publish` increments it and stores the value on the `otter_invalidation` row, which Pull delivers as the change's `stamp`. See [Server README](../server/README.md) for application wiring.
