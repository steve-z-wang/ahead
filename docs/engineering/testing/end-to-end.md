...
Reviewed 2026-09-14; tests read, not executed. Rows reworded 2026-09-15 for completion from receipts and `bash integration/e2e/run.sh` executed the same day (round-trip 3 tests, parity 1 test, all passed). The suite runs a real backend over a temporary PostgreSQL cluster with the Node client, the Dart client and the example CLI.
