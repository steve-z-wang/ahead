...
2. **Defects with reproductions waiting to become regressions.** [#33](https://github.com/zanminwang/ahead/issues/33) (direct write on a pending create; the random direct-write run stays ignored until fixed). [#32](https://github.com/zanminwang/ahead/issues/32) (page from a previous subscription) is fixed and its sim reproduction enabled. Semantic compiler errors reporting end-of-file ([#48](https://github.com/zanminwang/ahead/issues/48), [Validate §11](../architecture/compiler/validate.md)) and `Model.update<>` compiling but never succeeding ([#49](https://github.com/zanminwang/ahead/issues/49)) need focused regressions with their fixes. The existing fix for [#33](https://github.com/zanminwang/ahead/issues/33) is [PR #42](https://github.com/zanminwang/ahead/pull/42); do not duplicate that implementation.
