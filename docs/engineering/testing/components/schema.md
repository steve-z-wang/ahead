...
| Identity must be one or more non-nullable scalar fields; `ahead_` and `sqlite_` prefixes refused in any case; names unique ([Models](../../architecture/schema/models.md)) | [compiler.rs](../../../../crates/compiler/tests/compiler.rs) `rejects_invalid_identity`, `schema_and_mutations`, `rejects_reserved_model_names_at_the_declaration`, `semantic_errors_report_the_offending_declaration` (duplicate model, missing identity); [contracts.rs](../../../../crates/core/tests/contracts.rs) `field_default_and_record_stamp_round_trip_and_ahead_prefix_is_rejected` (`sqlite_x`, `SQLITE_x`, `Ahead_x` refused; `sqlitex`, `Sqlite`, `aheadx` accepted) | covered | Duplicate field names are refused by code but not asserted. |
...
| Reference `via` arity and field types must match the target identity; `onTargetDelete` is `delete` or `none` ([Relations](../../architecture/schema/relations.md)) | positive case in `relationships_bindings_and_dependency_metadata`; unknown `via` field and unknown `onTargetDelete` in `semantic_errors_report_the_offending_declaration` | partial | No negative case for arity or type mismatch. |
...
| Slot decoding: order by `(model, op)`, list slots greedy, stable rejection codes ([Mutations](../../architecture/schema/mutations.md)) | [server/tests/runtime.rs](../../../../crates/server/tests/runtime.rs) | covered for single slots | Adjacent slots with the same model and operation (the `RemoveEntries` shape in the fixture) have no assertion; the greedy result is a *potential risk* awaiting a decision, not a test to write yet. `Model.update<>` compiles and always fails on the server: a *defect* in the compiler's acceptance, no test. |
