...
| Shared limits are defined once; a page continues only when full and one change over the limit is refused ([Common §8](../../architecture/protocol/common.md#8-crosscutting-concepts)) | `shared_limits_are_defined_once_and_a_page_continues_only_when_full` against [live-messages.json](../../../../fixtures/protocol/live-messages.json) | covered | Verified 2026-09-15: `cargo test -p ahead-core --test contracts --locked` (18 tests). |
| Subscribe and acknowledgement frames: normalization, refusals, acknowledgement versus page ([Subscriptions](../../architecture/protocol/subscriptions.md)) | `live_frames_decode_as_acknowledgement_or_page_and_scopes_normalize` over the shared fixture | covered | The SDK-side acknowledgement checks remain until the live session moves to Rust ([#58](https://github.com/zanminwang/ahead/issues/58)). |
