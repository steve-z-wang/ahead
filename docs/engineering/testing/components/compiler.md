...
| Structural validation refuses bad identities, unknown prerequisites, duplicate versions, ambiguous inverses, mismatched references, bindings and sequences | `rejects_invalid_identity`, `rejects_dependency_typos`, `singular_inverse_requires_a_unique_foreign_key`; `structural_refusals_a_schema_author_is_likely_to_hit` (duplicate field, reference arity and type mismatch, unknown reference argument, ambiguous inverse, binding parent that is a list slot, sequence target model mismatch; each refused input has a valid twin differing only in that rule); invalid patch field in `semantic_errors_report_the_offending_declaration` | covered | Branches still without an assertion: unknown sequence relation path, prerequisite argument mismatches, duplicate slot and duplicate prerequisite. Verified 2026-09-14: `cargo test -p ahead-compiler --locked` (13 compiler, 5 CLI, 2 history, 3 parse). |
