...
| Batches settle in accepted-prefix order, with or without checkpoints to await (A5) | sim `a5_batches_settle_in_accepted_prefix_order`, `a5_immediately_settleable_batch_waits_for_the_earlier_batch`; [push.rs](../../../../crates/sqlite/tests/push.rs) `accepted_batches_only_settle_in_ready_prefix`, `immediately_settleable_batch_waits_for_the_earlier_waiting_batch` | covered | The immediate path ([#53](https://github.com/zanminwang/ahead/issues/53)) now stores a nothing-awaited marker and settles through the ordered walk; the SQLite test also covers duplicate and changed receipts and reopen without a resend. Visible state without authority is decided in [#52](https://github.com/zanminwang/ahead/issues/52); this test does not cover that separate contract. |
| A batch whose checkpoints are all unawaitable settles after earlier batches, rebuilding from the base and remaining pending edits | [query.rs](../../../../crates/sqlite/tests/query.rs) `transport_pulls_only_subscribed_channels_and_unawaitable_checkpoints_settle` (pending count only) | partial | [#52](https://github.com/zanminwang/ahead/issues/52) confirms the existing outcome: without new authority, the update can revert and the create can disappear. The earlier one-off reproduction remains in Settlement §11. Add named visible-row and pending-work assertions for no/unrelated subscriptions, subsequent delivery and unsubscribe; the contract is no longer blocked on a decision. |
