...
Use [Simulation](../simulation/README.md) for interactions across clients and server. The coverage review below records what the existing tests assert; missing tests are tracked in [#68](https://github.com/zanminwang/ahead/issues/68).
...
| A replay that no longer applies falls back to the before image silently | none | missing | Behavior is a *potential risk* in Writes §11; decide whether to surface it before writing an expectation ([#55](https://github.com/zanminwang/ahead/issues/55)). |
...
| Deletes, tombstones, claims and channel moves (D4, D5, D6) | `channel_claims_and_cross_channel_delete`, `delete_across_channels_keeps_a_tombstone_until_every_claim_confirms`, `move_between_channels_and_back`, [client.rs](../../../../crates/sqlite/tests/client.rs) `unsubscribe_drops_records_nobody_else_claims_and_restarts_from_zero` | covered | Child records moving with their parent are exercised only by the simulation's `MoveMembership` action inside random runs, where the invariants, not a D4 assertion, judge the outcome; no named scenario exists. |
| A change that fails validation is skipped and the cursor still advances | `original_bad_change_skip_policy_is_retained` | covered (behavior) | Whether skipping should be visible or fatal is a *potential risk* awaiting a decision ([#51](https://github.com/zanminwang/ahead/issues/51)); the report is dropped on the SDK path today. |
...
| `drop_mutation` refuses a frozen mutation | none | missing | Tests drop only unsent mutations. Add the refusal case; it is the boundary of the "poisoned batch" limitation whose recovery is decided in [#56](https://github.com/zanminwang/ahead/issues/56). |
