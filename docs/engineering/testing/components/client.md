...
Reviewed 2026-09-14 against the client architecture documents; tests read, not executed. Settlement rows updated 2026-09-15 after executing `cargo test -p ahead-sqlite --locked` (63 tests, all passed), which includes the new [settlement.rs](../../../../crates/sqlite/tests/settlement.rs) regressions; `cargo clippy --workspace --locked` and `cargo fmt --check` were clean. Tests in [sqlite/tests](../../../../crates/sqlite/tests) use real SQLite as the harness but assert engine rules, so they are counted here as component evidence; the SQLite contract itself is under [Storage and persistence](../integration/persistence.md). Simulation scenarios that establish the same clause across client and server are listed under [Simulation scenarios](../simulation/scenarios.md) and referenced here only where they add a clause.
...
| Frozen bytes are stable across retry and restart (P4) | `offline_queue_and_frozen_bytes_survive_restart_and_ack_waits_for_pull`; `p4_frozen_bytes_are_stable`; [session.rs](../../../../bindings/common/tests/session.rs) `rust_selects_transport_actions_and_reuses_frozen_request_on_retry` | covered | Bytes after a supported schema reconciliation with a populated queue: `populated_queue_survives_additive_reconciliation_with_frozen_bytes_unchanged` ([Storage and persistence](../integration/persistence.md)). |
...
| Behavior | Existing tests | Coverage | Gap and next step |
| --- | --- | --- | --- |
| Stale pages are no-ops; a page beyond the cursor cannot skip the gap; the cursor advances to `toCursor` (A2) | [downlink.rs](../../../../crates/sqlite/tests/downlink.rs) `original_bad_change_skip_policy_is_retained`; [stamp_scenarios.rs](../../../../crates/sqlite/tests/stamp_scenarios.rs) `redelivered_page_is_a_no_op`; sim `a2_pages_apply_only_in_cursor_order` | covered | A page from a previous subscription of the same channel ([#32](https://github.com/zanminwang/ahead/issues/32)) is fixed: [downlink.rs](../../../../crates/sqlite/tests/downlink.rs) `page_from_a_previous_subscription_is_stale_not_a_gap` covers `apply_page`, `receive_downlink` and `SyncCycle`, and the sim scenario is enabled. |
...
| Batches settle in accepted-prefix order, with or without checkpoints to await (A5) | sim `a5_batches_settle_in_accepted_prefix_order`, `a5_immediately_settleable_batch_waits_for_the_earlier_batch`; [push.rs](../../../../crates/sqlite/tests/push.rs) `accepted_batches_only_settle_in_ready_prefix`, `immediately_settleable_batch_waits_for_the_earlier_waiting_batch` | covered | The immediate path ([#53](https://github.com/zanminwang/ahead/issues/53)) now stores a nothing-awaited marker and settles through the ordered walk; the SQLite test also covers duplicate and changed receipts and reopen without a resend. Visible state without authority is a separate contract, decided in [#52](https://github.com/zanminwang/ahead/issues/52) and covered by [settlement.rs](../../../../crates/sqlite/tests/settlement.rs). |
| A batch whose checkpoints are all unawaitable settles after earlier batches, rebuilding from the base and remaining pending edits | [settlement.rs](../../../../crates/sqlite/tests/settlement.rs) `update_without_subscription_reverts_to_the_base_on_settlement`, `create_without_subscription_disappears_on_settlement`, `unrelated_subscription_does_not_await_the_checkpoint`, `remaining_pending_edits_replay_over_the_base`; [push.rs](../../../../crates/sqlite/tests/push.rs) `immediately_settleable_batch_waits_for_the_earlier_waiting_batch` (the "after earlier batches" clause); [query.rs](../../../../crates/sqlite/tests/query.rs) `transport_pulls_only_subscribed_channels_and_unawaitable_checkpoints_settle` (pending count and pull selection) | covered | none. [#52](https://github.com/zanminwang/ahead/issues/52) confirmed the existing outcome and the regressions now assert the visible records and the pending work: the update reverts to its base, the create disappears, an unrelated subscription's cursor is untouched, and a still-pending later edit replays over the base. |
| A later subscription delivers the server's result for a batch that settled without authority | [settlement.rs](../../../../crates/sqlite/tests/settlement.rs) `later_subscription_delivers_the_authoritative_result` | covered | none |
...
| Unsubscribing settles batches waiting on that channel | `unsubscribing_settles_its_checkpoint_and_later_pages_are_dropped`; [settlement.rs](../../../../crates/sqlite/tests/settlement.rs) `unsubscribe_releases_the_wait_and_removes_unclaimed_records` (visible rows before and after: the optimistic create is removed, the update reverts onto its base) | covered | none |
...
| Live session transitions: subscribe, acknowledgement before catch-up, held pages, one request at a time, full pages continue, gap and overflow recovery, subscription change, reconnect with backoff, protocol violations, pause, resume, stop, stale answers of an old session ([Live session](../../architecture/client/connection/controller/live-session.md)) | [sqlite/tests/live.rs](../../../../crates/sqlite/tests/live.rs) (8 tests, scripted host, real SQLite); the `live` command in [session.rs](../../../../bindings/common/tests/session.rs) `incoming_pages_share_cursor_policy_and_do_not_overwrite_push_cycle`, `incoming_overlap_is_identical_with_or_without_http_request_metadata` | covered | Verified 2026-09-15: `cargo test -p ahead-sqlite --test live --locked`, `cargo test -p ahead-binding --locked`. Real sockets, HTTP and credential refresh stay under [Connection integration](../integration/connection.md). |
