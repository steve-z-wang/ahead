...
| Reconciliation with a populated queue: frozen bytes unchanged, unsent creates still valid | none | missing | Both P4 and C3 cite this; the ddl tests run with an empty queue. Add a case that enqueues, freezes, reopens with an added nullable column, and compares bytes. |
