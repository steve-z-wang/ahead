...
| --- | --- | --- | --- |
| Command contract: session isolation, closed handles, transport actions, lanes, dispositions ([Bindings](../../architecture/sdks/bindings.md)) | [session.rs](../../../../bindings/common/tests/session.rs); `transaction_scoped_commands_and_sync_commands_are_refused_by_code` (a transaction-scoped read before and after a session is `transaction_closed`; `freeze`, `status` and `tasks` during a session are `client transaction active`; a closed handle is `client_closed`) | covered | none |
