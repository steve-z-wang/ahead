...
A shared Rust engine alone does not establish SDK equivalence. The coverage review below records what the existing tests assert; missing tests are tracked in [#68](https://github.com/zanminwang/ahead/issues/68).
...
| Errors cross as strings; HTTP mapping depends on text | none | not a test gap | Technical debt recorded in Bindings §11 and tracked in [#44](https://github.com/zanminwang/ahead/issues/44); a typed contract would come with its own tests. |
