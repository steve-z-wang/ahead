...
The CLI writes mutation history beside the schema, at `history/mutations.json` in the input directory; `--mutation-history FILE` overrides that location. Commit history to Git so subsequent compilation can retain old contracts. When the default is absent and a history exists at the superseded `OUTPUT_DIR/mutation-history.json`, the CLI reads it, writes the new location, leaves the old file in place and reports the move on stderr; it never reinitializes a history silently.
