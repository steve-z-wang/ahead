...
4. **History.** Version and compatibility rules per mutation ([Mutations](../schema/mutations.md)); this is the compile-time part of guarantee C3.
...
- **To confirm:** determinism (identical bytes for identical input) is assumed from source order and sorted maps but not asserted (guarantee S1 note).
