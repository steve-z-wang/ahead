...
6. **History.** Version and compatibility rules per mutation ([Mutations](../schema/mutations.md)); this checks mutation-history compatibility at compile time. The rule compares retained descriptor snapshots with the current descriptors, so it runs on the descriptor value from [Generate](generate.md), after `validate` returns, rather than on `Validated`.
...
Code: `validate` and the `Validated` types in [compiler/validate.rs](../../../../crates/compiler/src/validate.rs); `reconcile_history` and `check_fence` in [compiler/history.rs](../../../../crates/compiler/src/history.rs), called by [compiler/main.rs](../../../../crates/compiler/src/main.rs) on the descriptors.
