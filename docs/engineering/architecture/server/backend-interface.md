...
- A **handler** receives `{input, tx, userId, changes, publish}`: the decoded input (one value per slot), the transaction, the user id, the mutation's change set and a way to publish. `changes` starts as the records the uploaded operations target; `changes.add(record)` reports a record the handler changed beyond them. `publish({channel})` publishes the mutation's final change set to a channel; `publish({channel, records})` publishes exactly those records (`[]` nothing). Its return value is ignored. Throwing `MutationRejected`, or an error `translateRejection` maps to a code, rejects that one mutation; any other error aborts the whole batch. What the framework does with the change set after the handler returns is [Push](engine/push.md).
...
| `handle` | `name`, `version`, `arguments`, `owner`, `ordinal` | `Handled::Settled { changes: Vec<RecordRef>, publications: Vec<PublicationIntent { channel, records: Option<Vec<RecordRef>> }> }` or `Handled::Rejected { rejection }` |
| `load` | `model`, `version`, `identities`, `owner` | `Loaded::Rows(Vec<Option<Value>>)` (one entry per identity) or `Loaded::Refused { rejection }` |
...
- A `handle` answer carrying `changes` or `publications` beside `rejection`, or only one of `changes` and `publications`, is refused with `handler.invalid` ("invalid handler settlement"), as is a change or publication record without a model or an object identity. A `load` answer that is neither a row list nor `{rejection}` with a valid code is `loader.invalid`.
...
- **A rejection rolls back only its mutation; a loader refusal in a push does the same; any other error rolls back the batch** (guarantee P6). Evidence: `explicit rejection rolls back only mutation and its publication`, `a loader refusal during push rejects only that mutation; a loader defect aborts the batch`, `unknown error rolls back entire batch including earlier effects and client claim`, `registered translator rejects one mutation; malformed translator code aborts transaction`.
...
Executed 2026-09-15: `cargo test -p ahead-server --locked` passed with the Rust tests above; `bash integration/persistence/server/run.sh` (`runtime.test.mjs` and `host-contract.test.mjs`, 61 passed) executed the same day.
...
**The contract grew with readback ([#55](https://github.com/zanminwang/ahead/issues/55)).** `load` gained `version` for model read contracts ([#91](https://github.com/zanminwang/ahead/issues/91)) and lost `channel`; `handle` answers a change set and publications instead of a channel; `advanceStamp` and `ensureStamp` are new and `publish` takes the stamp instead of allocating one; `load` may answer a refusal. Twelve operations in all, each with one example in the fixture. Any further failure-result extension belongs to [#95](https://github.com/zanminwang/ahead/issues/95).
