...
**Server direction** (Node only). `validateConfig`, `processPush`, `processPull`, `publish`, `negotiateLive`, `pullLive`, each taking the config JSON, the owner, the request, and a host callback `(requestJson) => Promise<responseJson>` that runs inside the application's database transaction ([Backend interface](../server/backend-interface.md)). `negotiateLive` also opens the socket's live session and answers `{handle, actions}`; two synchronous functions then drive it without a host callback: `liveEvent(handle, eventJson) → actionsJson` applies one event to the Rust `Subscriptions` controller and returns the actions the host executes, and `liveClose(handle)` forgets the session (idempotent). The session outlives each call in a process-wide registry keyed by the handle ([Server / Connection / Controller](../server/connection/controller.md)); the host's own transactions for `pullLive` stay outside the controller. A failure is a structured engine error `{code, message, details?}`: `code` is a stable machine name, `message` is for people and may be reworded, and `details` carries only the fields a code promises (`mutation_version_unsupported` has `ordinal`, `name`, `version`). An event on a handle that is not open, or one the session cannot accept, is `live.invalid_event`. The engine codes are declared in [server/error.rs](../../../../crates/server/src/error.rs); the host callback's own failures cross back as text and are filed under `host` with their message kept.

## 5. Building Block View

- **The shared command host** (`RuntimeHost`) maps a numeric handle to an open client plus its connection state machines and dispatches every command to the [frontend interface](../client/frontend-interface.md). Command families: session (`begin`, `commit`, `rollback`, savepoints), reads, writes (`enqueue`, `direct`, `channel`), the push lane's scheduling (`connection`), the live lane (`live`: the events of the [live session](../client/connection/controller/live-session.md), answered with its actions), the push cycle (`startSync`, `next`, `complete`) and its protocol seams (`freeze`, `ack`, `pull`), and state (`readiness`, `drop`, `dismiss`, `recordStatus`, `tasks`, `status`).
...
- **React Native carrier.** The Expo iOS module dispatches string calls on a serial background queue to `ahead_mobile_call`, unwraps its result envelope, and frees the returned Rust string exactly once. The mobile carrier calls the same RuntimeHost; persistent database paths resolve under Application Support. Client failures reject promises with the existing message-based client contract. [Module and build](../../../../packages/client-react-native/native-module/README.md).
