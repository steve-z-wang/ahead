...
A write must be visible immediately and still be undoable. Writes achieves both by applying every change to the visible table at once and, for records that now have a pending mutation, remembering the last row the server sent in a *before image*. Later components rebuild the visible row from that before image when the server accepts, rejects or overrides the change.
...
All tests above were read, not executed.

## 11. Risks and Technical Debt

**Problem: a direct write on a pending-create record fabricates truth.** *Condition:* the record was created by a still-pending mutation (its before image is "absent"), and the application writes it directly. *Consequence:* the fold cannot apply an update to an absent before image and falls back to storing the current visible row, so when the create is rejected the rebuild resurrects the row with no claim and no pending mutation; the server never had it and nothing removes it. *Evidence:* `direct_one` in [client/mutate.rs](../../../../../../crates/client/src/mutate.rs); the random simulation with direct writes is `#[ignore]`d in [crates/sim/tests/invariants.rs](../../../../../../crates/sim/tests/invariants.rs). Tracked in [#33](https://github.com/zanminwang/ahead/issues/33).
