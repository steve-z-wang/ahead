...
**Accepted limitation.** Tombstone and stamp rows are retained until every claiming channel confirms; nothing bounds a channel that never does. Retention on both sides is [#61](https://github.com/zanminwang/ahead/issues/61). One transaction per change is a known cost ([#12](https://github.com/zanminwang/ahead/issues/12)) and the reason an atomic page hook ([#17](https://github.com/zanminwang/ahead/issues/17)) needs a design change.
