...
- Sync surface: `freeze`/`freeze_with_limit`, `acknowledge`, `apply_page`, `receive_downlink`, `downlink_request`, `SyncCycle`, `ConnectionDriver` ([Engine / Push](engine/push.md), [Engine / Pull](engine/pull.md), [Connection / Controller](connection/controller.md)).
...
- **Confirmed limitation: a sent mutation cannot be dropped.** `drop_mutation` refuses any mutation with a push number, so a batch the server keeps failing has no local escape hatch ([Engine / Push](engine/push.md)).
