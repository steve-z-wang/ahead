...
**Opening** runs the framework DDL, then in one transaction reconciles the model tables ([Reconciliation](storage/reconciliation.md)), creates or reads the client row (client id, ordinal and push counters, generation), and settles any batch whose checkpoints are already met. Opening does not change the generation, so two fresh handles are both valid until one of them writes.
...
**Accepted limitation.** `drop_mutation` refuses a mutation that has been frozen, because its outcome is unknown or accepted. The consequence for a batch the server keeps failing is recorded under [Batching](engine/push/batching.md).
