...
[English](../../architecture/code-organization.md) | [简体中文](code-organization.md)
...
可执行 Rust workspace 现为 `crates/{lfs-core,lfs-client,lfs-server,lfs-sqlite,lfs-compiler}` 加上 `bindings/{common,dart}`；`bindings/node` 有独立的 N-API build manifest。公开 host package 为 `packages/{client-js,dart,server,persistence-prisma,nest}`。跨组件测试位于 `integration/{rust,bindings,persistence,generated-api,nest,e2e,platform}`，可复用输入保留在 `fixtures/`。`scripts/test.sh` 运行 native host 检查；平台 simulator 测试有单独脚本。

Rust client 包含通用 Record 和 schema descriptor。语言生成器输出业务类型、编码/解码、强类型查询选项、关系访问器和 Mutation builder，不生成 replay、settlement 或 scheduling 算法。Host connection class 提供 timer/网络取消；Rust 选择动作和重试延迟。后端注册仍可使用普通函数或 Nest decorator，HTTP/WebSocket 挂载到应用管理的 server。

初版 SQLite adapter 存储发生变化的键控文档，并保留完整的内存状态快照。只读 SQL 在隔离的 SQLite connection 中查询该乐观快照。这些选择使第一版实现可以得到验证；在声称支持生产规模之前，专用 projection table/index 和大型缓存优化仍需要性能工作。
