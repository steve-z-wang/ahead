...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn savoia_call(input: *const c_char) -> *mut c_char {
...
/// # Safety
/// output must be a pointer returned by savoia_call, freed exactly once.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn savoia_free(output: *mut c_char) {
    if !output.is_null() {
