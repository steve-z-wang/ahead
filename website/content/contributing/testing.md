...
For focused checks, run `bash integration/persistence/transaction-probe/run.sh` or `bash integration/generated-api/verify.sh`. The capacity diagnostic is available through `cargo run -p savoia-integration --example capacity`.
