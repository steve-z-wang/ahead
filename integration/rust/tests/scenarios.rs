...
use savoia_core::{PullPage, PullRequest, PushReceipt, Schema};
use savoia_server::{Config, Host};
...
impl Backend {
    ...
    fn push(&self, body: &[u8]) -> String {
        ...
        match run(savoia_server::process_push(&config(), "u", body, self)) {
            Ok(r) => r,
            Err(e) => {
    ...
    fn pull(&self, client: &mut Client<SqliteStore>) -> PullPage {
        ...
        .unwrap();
        PullPage::decode(
            run(savoia_server::process_pull(&config(), "u", &body, self))
                .unwrap()
                .as_bytes(),
        )
        .unwrap()
...
impl Host for Backend {
 ...
 "head"=>json!(db.head),"savepoint"|"release"|"rollback"=>Value::Null,
...
fn deterministic_interleavings_preserve_local_priority_and_eventually_converge() {
    for seed in 0..64u64 {
        ...
        if seed & 16 != 0 {
            ...
            let body = client.freeze().unwrap().unwrap();
            let seq = savoia_core::PushRequest::decode(&body)
                .unwrap()
                .batch_sequence;
