...
test('Node SDK -> native Rust -> HTTP -> Rust backend -> Prisma -> SQLite, then Dart',async()=>{
 ...
 try{
  ...
  client=await Client.open({path:join(directory,'client.sqlite'),schema:app.schema,owner:'demo-user'});await client.subscribe('book:demo');await client.sync(transport);assert.equal((await client.read('Entry',{id:'entry-1'})).text,'Hello from the server');
  ...
  client=await Client.open({path:join(directory,'client.sqlite'),schema:app.schema,owner:'demo-user'});assert.equal(await client.freeze(),frozen);
  ...
  await new Promise((resolve,reject)=>{const child=spawn('dart',[`--packages=${join(root,'packages/dart/.dart_tool/package_config.json')}`,'../../integration/e2e/dart_client.dart',url,directory],{cwd:join(root,'packages/dart'),env:{...process.env,LFS_LIBRARY:join(root,'target/debug/liblfs_dart.dylib')},stdio:'inherit'});child.on('error',reject);child.on('exit',code=>code===0?resolve():reject(Error(`Dart E2E exited ${code}`)));});
