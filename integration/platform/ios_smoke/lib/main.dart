...
Future<void> main() async {
  ...
  final stageFile = File('${storage.path}/savoia-smoke-stage.txt');
  final resultFile = File('${storage.path}/savoia-smoke-result.txt');
  final expectedFile = File('${storage.path}/savoia-smoke-freeze.txt');
  final databasePath = '${storage.path}/savoia-smoke.sqlite';

  void stage(String value) {
    print('SAVOIA_SMOKE_STAGE $value temp=${storage.path}');
  ...
  try {
    ...
    if (expectedFile.existsSync()) {
      ...
      result = 'SAVOIA_SMOKE_RESTART_OK';
    } else {
      await bounded(
        'DIRECT_TX',
