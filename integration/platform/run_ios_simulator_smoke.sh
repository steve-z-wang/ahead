...
bundle_id="dev.localfirststate.lfsIosSmoke"
...
cargo build -p lfs-dart --target aarch64-apple-ios-sim
...
result_file="$data_container/tmp/lfs-smoke-result.txt"
stage_file="$data_container/tmp/lfs-smoke-stage.txt"
console_file="$data_container/tmp/lfs-smoke-console.txt"
: >"$console_file"
...
launch_app
wait_for_result LFS_SMOKE_PHASE1_OK
finish_launch
xcrun simctl terminate "$device_id" "$bundle_id"
launch_app
wait_for_result LFS_SMOKE_RESTART_OK
finish_launch
cat "$console_file"
