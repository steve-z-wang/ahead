...
bundle_id="dev.localfirststate.savoiaIosSmoke"
...
cargo build -p savoia-dart --target aarch64-apple-ios-sim
...
result_file="$data_container/tmp/savoia-smoke-result.txt"
stage_file="$data_container/tmp/savoia-smoke-stage.txt"
console_file="$data_container/tmp/savoia-smoke-console.txt"
: >"$console_file"
...
launch_app
wait_for_result SAVOIA_SMOKE_PHASE1_OK
finish_launch
xcrun simctl terminate "$device_id" "$bundle_id"
launch_app
wait_for_result SAVOIA_SMOKE_RESTART_OK
finish_launch
cat "$console_file"
