...
bundle_id="dev.localfirststate.otterIosSmoke"
...
cargo build -p otter-dart --target aarch64-apple-ios-sim
...
result_file="$data_container/tmp/otter-smoke-result.txt"
stage_file="$data_container/tmp/otter-smoke-stage.txt"
console_file="$data_container/tmp/otter-smoke-console.txt"
: >"$console_file"
...
launch_app
wait_for_result OTTER_SMOKE_PHASE1_OK
finish_launch
xcrun simctl terminate "$device_id" "$bundle_id"
launch_app
wait_for_result OTTER_SMOKE_RESTART_OK
finish_launch
cat "$console_file"
