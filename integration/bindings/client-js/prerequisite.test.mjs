...
test('prerequisite failure stays optimistic and explicit retry unlocks Rust push',async()=>{
 const directory=await mkdtemp(join(tmpdir(),'otter-prerequisite-'));const schema=JSON.parse(await readFile(new URL('../../../fixtures/schemas/entry.json',import.meta.url),'utf8'));schema.prerequisites=[{name:'Upload',fields:[{name:'key',type:'String'}]}];schema.requirements=[{model:'Entry',field:'note',name:'Upload',arguments:{key:'self'}}];
